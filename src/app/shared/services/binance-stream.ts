...
export class BinanceStream {
  ...
  private readonly connectionStatusSignal =
    signal<ConnectionStatus>('connecting');
  ...
  constructor() {
    this.ticker$ = this.tickerSubject.pipe(
      switchMap((ticker) => {
        ...
        this.socket = webSocket<BinanceRawTicker>(
        ...
        return this.socket.pipe(
          ...
          retry({
            delay: (_, retryCount) =>
              timer(Math.pow(2, Math.min(retryCount, 5)) * 1000),
          }),
          map(
            (data: BinanceRawTicker): CryptoTicker => ({
              symbol: data.s,
